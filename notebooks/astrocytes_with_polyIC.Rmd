...
```{r}
#protein data
raw_data <- read_tsv(file = '../data/combined_protein_polyIC_astrocytes.tsv')

#meta data
meta_data <- read_tsv(file = '../data/experiment_annotation_polyIC_astrocytes.tsv')
...
```{r}
#1. remove contamination
data_wo_contam <- raw_data[- grep("contam_sp", raw_data$Protein),]
...
data_numeric <- data_shorten %>% select(contains("MaxLFQ Intensity"))
rownames(data_numeric) <- data_shorten$Protein
data_logged <- log2(data_numeric)
rownames(data_logged) <- data_shorten$Protein

data_logged$na_count <- rowSums(is.na(data_logged))
naid <- which(data_logged$na_count == ncol(data_logged)-1)
data_logged <- data_logged[-naid,-25] 
...
imput_data <- llsImpute(data_logged, k=10, center = TRUE, completeObs = TRUE)
imput_dataframe <- as.data.frame(imput_data@completeObs)
hist(as.matrix(imput_dataframe), breaks = 100)

#method number 2
library(imputeLCMD)

svd <- as.data.frame(impute.wrapper.SVD(data_logged, K = 10))
hist(as.matrix(svd), breaks = 100)

#method number 3
data_bpca <- pca(data_logged, method = "bpca", center = TRUE, nPcs = 5, completeObs = TRUE, scale = "vector")

data_bpca <- as.data.frame(data_bpca@completeObs)
rownames(data_bpca) <- rownames(data_logged)
hist(as.matrix(data_bpca), breaks = 100)

#with filted data:
#method number 1
imput_data_filter <- llsImpute(data_filt, k=10, center = TRUE, completeObs = TRUE)
imput_dataframe_filter <- as.data.frame(imput_data_filter@completeObs)
hist(as.matrix(imput_dataframe_filter), breaks = 100)

#method number 2

svd_filter <- as.data.frame(impute.wrapper.SVD(data_filt, K = 10))
hist(as.matrix(svd_filter), breaks = 100)

#method number 3
data_bpca_filter <- pca(data_filt, method = "bpca", center = TRUE, nPcs = 5, completeObs = TRUE, scale = "vector")

data_bpca_filter <- as.data.frame(data_bpca_filter@completeObs)
rownames(data_bpca_filter) <- rownames(data_filt)
hist(as.matrix(data_bpca_filter), breaks = 100)
...
desMat <- model.matrix(~ polyIC10ug + polyIC1ug + lipo, female_meta_data)
fit <- lmFit(female_logged_data, desMat)
colnames(fit)
tmp <- eBayes(fit)
res <- topTable(tmp, n = Inf, coef = "polyIC10ugYES") 
female_all <- topTable(tmp, n = Inf, coef = c("polyIC10ugYES", "polyIC1ugYES", "lipoYES"))
female_results <- subset(female_all, n = Inf, female_all$adj.P.Val < 0.05)
