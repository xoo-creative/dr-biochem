...
astro_gprofiler <- read_csv(file='../data/20240226_gprofiler_sig.csv')

EC$david

#GO analysis for female
female_sigprots <- rownames_to_column(female_sigprots, var = "gene")
...
```{r}
astro_circ <- circle_dat(astro_gprofiler, astro_sigprots_names)

## Filtering those with `adj_pval` == 1, cause it's noise that clogs up the plot
astro_circ <- subset(astro_circ, adj_pval < 1)
```
...
GOHeat(chord, nlfc = 0) # 0 for plotting counts
