...
```{r}
plot_volcano(dep, contrast = "control231_vs_drug231", label_size = 2, add_names = TRUE)
```

```{r}
plot_volcano(dep, contrast = "control468_vs_drug468", label_size = 2, add_names = TRUE)
```

```{r}
plot_volcano(dep, contrast = "high231_vs_high_drug231", label_size = 2, add_names = TRUE)
```

```{r}
plot_volcano(dep, contrast = "low468_vs_low_drug468", label_size = 2, add_names = TRUE)
```

#### Does the drug have a diffferent effect on control cell lines vs engineered cell lines?

```{r}
plot_volcano(dep, contrast = "drug468_vs_low_drug468", label_size = 2, add_names = TRUE)
```

```{r}
plot_volcano(dep, contrast = "drug231_vs_high_drug231", label_size = 2, add_names = TRUE)
```
