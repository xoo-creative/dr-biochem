...
combined_proteins_file="20240226_report.pg_matrix_pri_astro_sex_original.tsv"
experiment_annotation_file="20240226_experiment_annotation_DIA_pri_astro_sex.tsv"
...
## Get diff expressd proteins:

```{r}
protein_table <- get_results(dep)

sig.prots <- protein_table |> filter(significant == TRUE)

write_csv(protein_table, "sig_prots_20240316.csv")
```
...
```{r}
plot_volcano(dep, contrast = "female_vs_male", label_size = 2, add_names = TRUE)
```
...
```{r}
GetProteinFunction("A0MZ66")[,"Function..CC."]
```

```{r}
Pathway.Enr(sig.prots_short$ID)
```

```{r}
Enrichment.REAC(sig.prots$ID, top = 20)
```

```{r}

sig.prots_short <- sig.prots[order(sig.prots$female_vs_male_p.adj),] |> head(50)

sig.

pathology_obj <- GetPathology_Biotech(sig.prots_short$ID)
diseases <- Get.diseases(pathology_obj)

sig.prots_short$diseases = diseases$Disease
```

```{r}
prots_info = GetProteinFunction(sig.prots_short$ID)
sig.prots_short$Function = prots_info$Function..CC.
